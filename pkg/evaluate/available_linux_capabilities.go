		...
		if len(lst) == 2 {
			...
			for _, c := range caps {
				...
				case "CAP_DAC_READ_SEARCH":
					fmt.Println("[!] CAP_DAC_READ_SEARCH enabled. You can read files from host. Use 'k8strike run cap-dac-read-search' ... for exploitation.")
				case "CAP_SYS_MODULE":
				...
				case "CAP_SYS_ADMIN":
					fmt.Println("Critical - SYS_ADMIN Capability Found. Try 'k8strike run rewrite-cgroup-devices/mount-cgroup/...'.")
				}
			}
		}
